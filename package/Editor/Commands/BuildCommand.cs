...
namespace UnityBridge.Commands {
    public class BuildCommand : ICommand {
        ...
        public void Execute(
            ...
            Debug.Log($"{AgentsBridge.LogPrefix} Build command received - method: {method ?? "direct"}");
            ...
            try {
                ...
                stopwatch.Stop();
                Debug.LogError($"{AgentsBridge.LogPrefix} Build error: {e.Message}");
                onComplete?.Invoke(
        ...
        private void ExecuteDirectBuild(
            ...
            Debug.Log($"{AgentsBridge.LogPrefix} Starting direct build: target={target}, dev={isDev}, output={outputPath}");
            ...
            if (report.summary.result == BuildResult.Succeeded) {
                Debug.Log($"{AgentsBridge.LogPrefix} Build succeeded: {report.summary.totalSize} bytes in {buildInfo.totalSeconds:F1}s");
                response = CommandResponse.Success(request.id, request.action, stopwatch.ElapsedMilliseconds);
            } else {
                Debug.LogError($"{AgentsBridge.LogPrefix} Build failed: {report.summary.result}");
        ...
        private void ExecuteMethodBuild(
            ...
            if (lastDot <= 0) {
                onComplete?.Invoke(CommandResponse.Error(
                    request.id, request.action,
                    $"Invalid method format: '{methodPath}'. Expected 'Namespace.Class.Method' (e.g., 'MyProject.Builder.BuildEntryPoints.BuildQuest')."
                ));
                return;
            }
            ...
            Debug.Log($"{AgentsBridge.LogPrefix} Invoking build method: {methodPath}");
            ...
            try {
                ...
                Debug.Log($"{AgentsBridge.LogPrefix} Method build completed in {buildInfo.totalSeconds:F1}s");
                ...
                Debug.LogError($"{AgentsBridge.LogPrefix} Method build threw exception: {inner.Message}");
