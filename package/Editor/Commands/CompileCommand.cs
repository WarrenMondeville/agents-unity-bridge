...
namespace UnityBridge.Commands {
    public class CompileCommand : ICommand {
...
#if DEBUG
            UnityEngine.Debug.Log(AgentsBridge.LogPrefix + " Starting script compilation");
#endif
        ...
        private void OnAssemblyCompilationFinished(string assemblyPath, CompilerMessage[] messages) {
            foreach (var message in messages) {
                if (message.type == CompilerMessageType.Error) {
                    _hasErrors = true;
                    UnityEngine.Debug.LogError($"{AgentsBridge.LogPrefix} Compilation error: {message.message}");
                }
            }
        }
        ...
        private void OnCompilationFinished(object context) {
            ...
            _stopwatch.Stop();
#if DEBUG
            UnityEngine.Debug.Log($"{AgentsBridge.LogPrefix} Compilation finished - HasErrors: {_hasErrors}");
#endif
