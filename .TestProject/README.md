...
This is a minimal Unity 2021.3+ project used for end-to-end integration testing of the Agents Unity Bridge package.
...
├── .agents-unity-bridge/       # Bridge protocol directory (gitignored, created at runtime)
...
The project references the Agents Unity Bridge package via a **local package reference**:

```json
{
  "dependencies": {
    "com.agents-unity-bridge": "file:.."
  }
}
```
...
2. Find "Agents Unity Bridge" in the package list
...
**Cause**: The `.agents-unity-bridge/` directory doesn't exist yet.

**Fix**:
1. Ensure Unity Editor is open with .TestProject loaded
2. The package should create `.agents-unity-bridge/` automatically on startup
3. Check Unity Console for any `[AgentsBridge]` errors
...
**Fix**:
1. Check `Packages/manifest.json` contains `"com.agents-unity-bridge": "file:.."`
...
**Fix**:
1. Check Unity Console for `[AgentsBridge]` initialization messages
2. Verify no errors during package initialization
3. Try restarting Unity Editor
4. Check if `.agents-unity-bridge/` directory exists and is writable
...
4. **Check Logs**: Monitor Unity Console for `[AgentsBridge]` messages
...
# Edit files in Editor/Commands/, Editor/AgentsBridge.cs, etc.
...
Everything else (Assets/, Library/, Temp/, Logs/, .agents-unity-bridge/) is gitignored.
